...
function formatAvailability(row: ProductRow): string {
  const total = row.variants.length;
  if (total === 0) return "—";
  const inStockCount = row.variants.filter((v) => v.inStock).length;
  if (inStockCount === 0) return "Out of stock";
  if (inStockCount === total) return `${total}/${total} in stock`;
  return `${inStockCount}/${total} in stock`;
}
...
function variantToFormState(
  v: ProductRow["variants"][number],
  product?: ProductRow
): VariantFormState {
  let existingImages = v.variantImages;
  if (
    existingImages.length === 0 &&
    product?.displayImage &&
    product.variants.length === 1
  ) {
    existingImages = [product.displayImage];
  }
...
export default function ProductsPage() {
  ...
  function populateEditForm(product: ProductRow) {
    setEditing(product);
    setFormName(product.productName);
    setFormCategoryId(product.categoryId);
    setFormDescription(product.productDescription ?? "");
    setFormGender(product.gender);
    setFormStatus(product.status);
    setFormBrand(product.brand ?? "");
    setFormVariants(
      product.variants.length > 0
        ? product.variants.map((v) => variantToFormState(v, product))
        : [emptyVariant()]
    );
  }
  ...
  async function openEdit(row: ProductRow) {
    ...
    try {
      ...
      setFormLoadWarning("Could not refresh product from server. Showing cached data.");
    } finally {
      setFormLoading(false);
    }
  }
  ...
  async function buildVariantImages(variant: VariantFormState): Promise<string[]> {
    const uploaded = await Promise.all(
      variant.pendingFiles.map(async (file) => {
        const dataUrl = await fileToDataUrl(file);
        return uploadImageBase64(dataUrl, "products");
      }),
    );
    const normalizedExisting = variant.existingImages.map(normalizeStoredAssetUrl);
  ...
  const maxVariantImageCount = (row: ProductRow) =>
    row.variants.reduce((max, variant) => Math.max(max, variant.variantImages.length), 0);
                      ...
                      <TableCell className="px-5 py-4 text-sm">
                        {firstImage(p) ? (
                          <div className="flex flex-col items-start gap-1">
                            ...
                            />
                            {maxVariantImageCount(p) > 1 && (
                              <span className="text-[10px] text-gray-500 dark:text-gray-400">
                                {maxVariantImageCount(p)} photos
                              </span>
                            )}
                          </div>
                        ) : (
      ...
      <Modal isOpen={formOpen} onClose={closeForm} className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4">
            ...
            {formLoadWarning && (
              <p className="text-sm text-amber-600 dark:text-amber-400">{formLoadWarning}</p>
            )}
            {formError && (
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button type="submit" disabled={formSubmitting || formLoading}>
